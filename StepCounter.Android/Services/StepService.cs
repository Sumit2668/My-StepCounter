...
namespace StepCounter.Services
{
    ...
    [IntentFilter(new []{ "com.refractored.mystepcounter.StepService" })]
    public class StepService : Service, ISensorEventListener, INotifyPropertyChanged
    {
        ...
        void Startup(bool warning = false)
        {
        ...
        {
            bool notification = ProgressNotifications;

            const int halfGoal = 5000;
            const int fullGoal = 10000;
            const int doubleGoal = 20000;
            if (stepsToday < halfGoal && stepsToday + newSteps >= halfGoal)
            {
                GoalTodayDay = DateTime.Today;
                GoalTodayMessage = Resources.GetString(Resource.String.goal_half);
            }
            else if (stepsToday < fullGoal && stepsToday + newSteps >= fullGoal)
            {
                GoalTodayDay = DateTime.Today;
                GoalTodayMessage = string.Format(Resources.GetString(Resource.String.goal_full), (fullGoal).ToString("N0"));
            }
            else if (stepsToday < doubleGoal && stepsToday + newSteps >= doubleGoal)
            {
                GoalTodayDay = DateTime.Today;
                GoalTodayMessage = string.Format(Resources.GetString(Resource.String.goal_double), (doubleGoal).ToString("N0"));
            }
            else
            {
                notification = false;
            ...
            if (notification)
            {
                PopUpNotification(0, Resources.GetString(Resource.String.goal_update), GoalTodayMessage);
            }

            notification = false;
            if (stepsToday + newSteps > HighScore)
            {
                HighScore = stepsToday + newSteps;
                //if not today
                if (!TodayIsHighScore)
                {
                    //if first day of use then no notifications, else pop it up
                    if (FirstDayOfUse.DayOfYear == DateTime.Today.DayOfYear &&
                        FirstDayOfUse.Year == DateTime.Today.Year)
                    {
                        notification = false;
                    }
                    else
            ...
            if (notification)
            {
                PopUpNotification(1, Resources.GetString(Resource.String.high_score_title),
                    string.Format(Resources.GetString(Resource.String.high_score),
                        Utils.FormatSteps(Settings.HighScore)));
            }

            notification = AccumulativeNotifications;
            var notificationString = string.Empty;
            if (TotalSteps + newSteps > Settings.NextGoal)
            {
        ...
        void PopUpNotification(int id, string title, string message)
        {
        ...
        private void CrunchDates(bool startup = false)
        {
            if (!Utils.IsSameDay)
            {
                ...
                dayEntry.Steps = CurrentDaySteps;

                CurrentDay = DateTime.Today;
                CurrentDaySteps = 0;
                StepsBeforeToday = TotalSteps;
                StepsToday = 0;
                try
                {
            ...
            {
                StepsToday = TotalSteps - StepsBeforeToday;
            }
        }
